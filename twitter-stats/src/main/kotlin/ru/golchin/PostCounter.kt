    ...
    fun calculatePostCounts(hashtag: String, hours: Int): Array<Long> {
        ...
        repeat(hours) {
            ...
            client.doRequest("newsfeed.search", requestParams, 3) { stream ->
                ...
                val postCount = postPage.postCount
                assert(postCount >= 0) { "negative post count not allowed: $postCount" }
                if (postPage.errorMessage != null)
                    throw ServiceInvocationException(postPage.errorMessage)
                totalCount += postPage.postCount
                requestParams["start_from"] = postPage.nextId ?: return@doRequest false
                true
            }
            result[result.size - it - 1] = totalCount
        }
        return result
    }
}
...
fun main(args: Array<String>) {
    PostCounter().calculatePostCounts("aa", 10)
}